        ...
        <article class="resource-item" data-path="${escapeHtml(
          ...
          <div class="resource-actions">
            <div class="install-dropdown copy-install-dropdown" data-skill-id="${escapeHtml(
              item.id
            )}" data-method="${INSTALL_METHODS[0].id}">
              ...
              <div class="install-dropdown-menu" role="menu">
                <div class="copy-install-method" role="group" aria-label="Install method">
                  ${INSTALL_METHODS.map(
                    (method, i) =>
                      `<button type="button" class="copy-install-method-btn${
                        i === 0 ? " is-active" : ""
                      }" data-method="${escapeHtml(method.id)}">${escapeHtml(
                        method.label
                      )}</button>`
                  ).join("")}
                </div>
