...
// Re-exported for existing importers; canonical values live in ../repo-config.
export { REPO_IDENTIFIER };

/**
 * Supported IDEs for the per-tool "Copy Install" dropdowns. The `id` maps to a
 * GitHub CLI `--agent` value understood by `gh skill install` (see
 * INSTALL_TOOL_AGENTS below).
 */
...
/**
 * Maps each tool id to its `gh skill install --agent` identifier. `copilot`
 * maps to `github-copilot`, which is the CLI default, so its flag is omitted.
 */
const INSTALL_TOOL_AGENTS: Record<string, string> = {
...
/**
 * Build the `gh skill install` command that lays a skill down in a given tool's
 * native format. Requires GitHub CLI >= v2.90.0. The default agent
 * (github-copilot) needs no `--agent` flag.
 */
export function getSkillInstallCommand(tool: string, skillId: string): string {
  const agent = INSTALL_TOOL_AGENTS[tool] ?? tool;
  const base = `gh skill install ${REPO_IDENTIFIER} ${skillId}`;
  return agent === "github-copilot" ? base : `${base} --agent ${agent}`;
}

/**
 * Host-agnostic alternative to `gh skill`: sparse-clone the pre-built native
 * files for this tool from `installs/<tool>/<skill>/` and copy them into the
 * current project. Works against any git host (no GitHub index, no gh CLI).
 * The artifacts are generated by `eng/build-installs.mjs`.
 */
export function getSkillGitCloneCommand(tool: string, skillId: string): string {
  const dir = `installs/${tool}/${skillId}`;
  const tmp = ".dvt-skill-tmp";
  return (
    `git clone --no-checkout --depth 1 ${REPO_HTTPS_URL} ${tmp} && ` +
...
/** Install-command builders keyed by the dropdown's selected method. */
export const INSTALL_METHODS = [
  { id: "gh", label: "gh skill CLI", build: getSkillInstallCommand },
  { id: "git", label: "git clone", build: getSkillGitCloneCommand },
] as const;

export type InstallMethodId = (typeof INSTALL_METHODS)[number]["id"];

/** Build the install command for a given method id (falls back to gh skill). */
export function getSkillCommandForMethod(
  method: string,
  tool: string,
  skillId: string
): string {
  const entry =
    INSTALL_METHODS.find((m) => m.id === method) ?? INSTALL_METHODS[0];
  return entry.build(tool, skillId);
}
